...
def get_dynamic_batch_size(device_load, base_batch_size=32, memory_limit_gb=2.0):
    ...
    try:
        if device_load["device"] == "GPU":
            memory_allocated = float(device_load.get("gpu_memory_allocated_gb", 0.0))
            if memory_allocated > memory_limit_gb:
                ...
                return get_new_batch_size(memory_limit_gb, base_batch_size)
            
        elif device_load["device"] == "CPU":
            memory_available = float(device_load.get("memory_available_gb", 0.0))
        ...
        logger.warning(f"Error al ajustar batch size: {e}. Device load: {device_load}")
